...
function graphRows(items: SessionGitHistoryEntry[]) {
  ...
  for (const item of items) {
    ...
    if (lane === -1) {
      lane = 0
      lanes.splice(lane, 0, item.oid)
    }

    const before = lanes.map((_, index) => index)
    let next = [...lanes]
    ...
    const after = next.map((_, index) => index)
    const parents = item.parents.map((parent) => next.indexOf(parent)).filter((index) => index >= 0)
    const rowWidth = Math.max(before.length, after.length, lane + 1, ...parents.map((index) => index + 1))
      ...
      <div class="flex items-center justify-between gap-2 border-b border-border-weak-base px-3 py-2">
        <div class="min-w-0 flex flex-col gap-1">
          <div class="flex min-w-0 items-center gap-2">
            ...
            <div class="text-11-regular text-text-weaker">{props.commits.length}</div>
          </div>
      ...
      >
        ...
        >
          <div class="min-h-0 flex-1 overflow-y-auto" data-scrollable>
            <div class="grid grid-cols-[88px_minmax(0,1fr)_110px_90px] border-b border-border-weak-base px-3 py-2 text-[11px] font-medium text-text-weaker">
              <div>Graph</div>
              <div>Commit</div>
              <div>Author</div>
              <div>Time</div>
            ...
            <For each={props.commits}>
              {(item, index) => {
                ...
                <button
                  type="button"
                  class="group grid h-[68px] w-full grid-cols-[88px_minmax(0,1fr)_110px_90px] items-stretch gap-0 border-b border-border-weak-base text-left transition-colors hover:bg-surface-raised-base-hover"
                  classList={{
