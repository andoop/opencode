...
OpenCode can use your local Cursor CLI installation as an agent backend.
...
3. Select **Cursor CLI** inside OpenCode.

4. Choose the `auto` model and start your session.

OpenCode forwards its session context and prompts to Cursor CLI, and exposes additional OpenCode tools to Cursor through a local MCP bridge.
...
- MCP tools that are already configured in OpenCode
