---
title: Providers
description: Using any LLM provider in RealseeCode.
---

import config from "../../../config.mjs"
export const console = config.console

RealseeCode uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your RealseeCode config.
...
You can customize the providers through the `provider` section in your RealseeCode
config.
...
RealseeCode Zen is a list of models provided by the RealseeCode team that have been
tested and verified to work well with RealseeCode. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with RealseeCode Zen.
:::
...
It works like any other provider in RealseeCode and is completely optional to use.
...
To use Amazon Bedrock with RealseeCode:
...
:::info
Using your Claude Pro/Max subscription in RealseeCode is not officially supported by [Anthropic](https://anthropic.com).
:::
...
RealseeCode can use your local Cursor CLI installation as an agent backend.

1. Install Cursor CLI and make sure the `agent` command is available in your shell.
...
3. Select **Cursor CLI** inside RealseeCode.

4. Choose the `auto` model and start your session.

RealseeCode forwards its session context and prompts to Cursor CLI, and exposes additional RealseeCode tools to Cursor through a local MCP bridge.

The first release focuses on high-value bridged tools:

- file read and search tools
- shell execution
- edit and write tools
- MCP tools that are already configured in RealseeCode

If Cursor CLI is not installed, this provider will not appear in the provider list.
...
RealseeCode uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock RealseeCode
...
To use Google Vertex AI with RealseeCode:
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each RealseeCode conversation listed as a separate session.
...
To use Ollama Cloud with RealseeCode:
...
3. Copy the API key for use in RealseeCode.
...
6. **Important**: Before using cloud models in RealseeCode, you must pull the model information locally:
...
RealseeCode Zen is a list of tested and verified models provided by the RealseeCode team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>RealseeCode Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **RealseeCode Zen**.

   ```txt
   /connect
   ```

3. Enter your RealseeCode API key.
...
The `limit` fields allow RealseeCode to understand how much context you have left. Standard providers pull these from models.dev automatically.
