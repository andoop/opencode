...
You can also disable commands through the `commands` config. This works for built-in system commands and custom commands.
...
Use the slash command name for built-in commands like `help` or `share`, and the command name for custom commands like `test`.
