...
Use the `commands` option to enable or disable commands by name, including built-in system commands and custom commands.
...
For system commands, use the slash command name like `help` or `share`. For custom commands, use the command name like `test`.
