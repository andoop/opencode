...
test("defaultRegisteredPermission enables ask only with cursor-cli whitelist", () => {
  ...
  expect(User.modelEnabled("cursor-cli/composer-2.5", { role: "user", permission })).toBe(true)
  expect(User.modelEnabled("cursor-cli/composer-2.5-fast", { role: "user", permission })).toBe(true)
  ...
  expect(User.modelEnabled("cursor-cli/auto", { role: "user", permission })).toBe(false)
