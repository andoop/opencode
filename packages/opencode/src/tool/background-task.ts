...
export const BackgroundTaskTool = Tool.define<typeof parameters, Record<string, unknown> & { taskId?: string }>(
  "background-task",
  {
    description:
      "Start a background task that runs asynchronously. Returns immediately with a task ID; use task_status to check progress.",
    parameters,
