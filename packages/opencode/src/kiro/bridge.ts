...
const blog = Log.create({ service: "kiro-bridge" })
...
export function bridgeCommand(input: { cwd: string; sessionID: string; agent: string; allowedTools: string[] }) {
  ...
  const readyFile = path.join(os.tmpdir(), `opencode-kiro-bridge-ready-${input.sessionID}-${Date.now()}`)
  const env = {
    ...inherited,
    OPENCODE_KIRO_ALLOWED_TOOLS: JSON.stringify(input.allowedTools),
    OPENCODE_KIRO_SESSION_ID: input.sessionID,
    OPENCODE_KIRO_AGENT: input.agent,
    OPENCODE_BRIDGE_READY_FILE: readyFile,
    XDG_STATE_HOME: process.env.XDG_STATE_HOME || path.join(os.tmpdir(), "opencode-kiro-xdg"),
  }
  const script = fileURLToPath(new URL("../index.ts", import.meta.url))
  if (process.env.OPENCODE_KIRO_BRIDGE_COMMAND) {
    const [command, ...args] = process.env.OPENCODE_KIRO_BRIDGE_COMMAND.split(" ")
    return { command, args: [...args, "--cwd", input.cwd], env, readyFile }
  }
  if (process.execPath.toLowerCase().includes("bun")) {
    return {
      command: process.execPath,
      args: ["run", "--conditions=browser", script, "kiro-bridge", "--cwd", input.cwd],
      env,
      readyFile,
    }
  }
  return {
    command: process.execPath,
    args: ["kiro-bridge", "--cwd", input.cwd],
    env,
    readyFile,
  }
}
