...
function kiroPath() {
  return process.env.OPENCODE_KIRO_CLI_PATH || Bun.which("kiro-cli")
}
      ...
      if (
        ...
        if (msg.error) {
          ...
          this.slog.warn("acp rpc response error", {
            ...
            failureKind: classifyFailure(detail),
          })
...
export namespace KiroCLI {
  export function available() {
    return Boolean(kiroPath())
  }
  ...
  export async function stream(input: {
    ...
    const bin = kiroPath()
    if (!bin) {
      throw new Error("Kiro CLI not found. Install it and ensure the `kiro-cli` command is available.")
    }
    ...
    if (proc.stderr) {
      ...
      errRl.on("line", (line) => {
        ...
        slog.warn("kiro agent stderr", { stderrLine: stderrLines, text: truncate(t, 500) })
      })
    }
    let textOpen = false
    ...
    function traceSummary(id: string) {
      ...
      return {
        toolName: toolNames.get(id) ?? "kiro_tool",
        status: item.status ?? "",
        title: item.title ?? "",
    ...
    async function waitForTrailingToolUpdates(reason: string, quietMs = 2000, maxWaitMs = 6000) {
      ...
      slog.warn("kiro-cli waiting for trailing tool updates", {
        reason,
        quietMs,
        maxWaitMs,
      ...
      slog.info("kiro-cli trailing tool wait completed", {
        reason,
    ...
    function settleOpenToolCalls(reason: string) {
      for (const id of seenToolCall) {
        ...
        const name = toolNames.get(id) ?? "kiro_tool"
        const trace = traceSummary(id)
        slog.warn("kiro-cli unfinished tool settled", {
          toolCallId: id,
          toolName: name,
    ...
    function logSessionUpdate(update: any) {
      ...
      if (!seenUpdateKinds.has(kind)) {
        ...
        slog.info("kiro session update kind", {
          kind,
      ...
      if (kind !== "tool_call" && kind !== "tool_call_update") {
        if (kind.includes("tool") || kind.includes("permission")) {
          slog.info("kiro session update detail", {
            kind,
            toolCallId: update.toolCallId ?? "",
            status: update.status ?? "",
            title: update.title ?? "",
    ...
    proc.on("exit", (code, signal) => {
      ...
      slog.warn("kiro agent process exited", {
        exitCode: code ?? "",
        signal: signal ?? "",
    ...
    const textId = "kiro-text"
    const reasoningId = "kiro-reasoning"
    const rpc = new Rpc(
      proc,
      (msg) => {
        ...
        switch (update.sessionUpdate) {
          case "agent_message_chunk": {
            ...
            if (!text) return
            roundText += text
            if (!textBuffered && roundText.includes(`<${KiroToolCall.TAG}`)) {
              textBuffered = true
              if (textOpen) {
            ...
            const tagOpen = `<${KiroToolCall.TAG}`
          ...
          case "tool_call_update": {
            ...
            if (!seenToolCall.has(id) && !toolInputs.has(id) && toolInput(update) === undefined) {
              slog.warn("kiro tool update missing initial input", {
                toolCallId: id,
                toolName: name,
                status: update.status ?? "",
              })
            ...
            if (update.status === "failed") {
              ...
              slog.warn("kiro tool_call_update failed", {
                toolCallId: id,
                toolName: name,
                detail: truncate(toolErrMsg, 400),
                failureKind: classifyFailure(toolErrMsg),
              })
              queue.push({
                type: "tool-error",
                toolCallId: id,
    ...
    const text = (async () => {
      try {
        ...
        function finish(done: string, reason = "stop") {
          settleOpenToolCalls(
            "Kiro ended the turn before this tool finished (common with terminal commands or disconnect).",
          )
          queue.push({
            type: "finish-step",
            finishReason: reason,
        ...
        slog.info("kiro initialize completed", {
          authMethods: (init?.authMethods ?? []).map((item: any) => item.methodId).join(","),
        })
        const kiroLogin = init?.authMethods?.find((item: any) => item.methodId === "kiro_login")
        if (kiroLogin) {
          await rpc.request("authenticate", { methodId: "kiro_login" }).catch((error) => {
            log.debug("kiro authenticate failed", { error })
          })
        }
        ...
        slog.info("kiro session created", {
        ...
        if (target && target.id !== session.modes?.currentModeId) {
          slog.info("kiro session set mode", {
            kiroSessionID: sessionId,
            modeId: target.id,
        ...
        // Wait for the MCP bridge subprocess to finish registering tools with Kiro.
        if (bridge.readyFile) {
          const deadline = Date.now() + 10_000
          while (Date.now() < deadline) {
            if (await Bun.file(bridge.readyFile).exists()) {
              slog.info("kiro bridge ready", { waitedMs: Date.now() - (deadline - 10_000) })
              break
            }
        ...
        while (true) {
          ...
          slog.info("kiro session prompt start", {
            kiroSessionID: sessionId,
          ...
          if (parsed.calls.length > 0 || parsed.errors.length > 0) {
            ...
            steps += 1
            if (steps >= KiroToolCall.STEP_MAX) {
              emitText("Kiro CLI tool loop reached the maximum number of tool steps.")
          ...
          slog.info("kiro session prompt completed", {
            kiroSessionID: sessionId,
        ...
        slog.error("kiro-cli stream failed", {
    ...
    input.abort.addEventListener(
      "abort",
      () => {
        slog.info("kiro-cli stream aborted by client", {
          duration: Date.now() - streamStartedAt,
