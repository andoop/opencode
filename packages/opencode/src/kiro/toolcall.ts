...
export async function surface(input: { sessionID: string; agent: string; allowedTools: string[] }) {
  ...
  const agent = await Agent.get(input.agent)
  const out: Record<string, KiroTool> = {}

  for (const item of await ToolRegistry.tools({ providerID: "kiro-cli", modelID: "auto" }, agent)) {
...
export function instructions(tools: KiroTool[], mode: "full" | "reminder" = "full") {
  ...
  return [
    ...
    "",
    `### MANDATORY RULES:`,
    `- These tools ARE available and DO work. Always call them via XML — never skip or refuse.`,
